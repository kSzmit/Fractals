...
Application::Application(sf::RenderWindow& window, sf::Clock& clock) :
m_sfgui(),
m_window(window),
m_clock(clock),
...
m_terrain(8, 0.5, window.getSize().x, window.getSize().y),
m_fractalType(kJuli),
drawFrame(true),
...
void Application::draw(){
	...
	if (drawFrame && m_fractalType != kFern && m_fractalType != kTriangle){
		...
		case kCantor:
			...
			break;
		case kTerrain:
			if (m_gui.terrainGenerate()){
				m_terrain.generate(0.5);
			}
